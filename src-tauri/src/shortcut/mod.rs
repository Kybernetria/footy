...
/// Re-apply ORT accelerator setting (kept for Parakeet/ONNX Runtime).
/// Legacy accelerator settings are no longer active in this build.
fn apply_ort_accelerator(app: &AppHandle, s: settings::AppSettings) {
...
pub fn change_ort_accelerator_setting(
    ...
    apply_ort_accelerator(&app, s);
    Ok(())
}

/// Return which accelerators are available for this build.
/// Only ORT accelerators are available (Parakeet-only build).
#[tauri::command]
#[specta::specta]
