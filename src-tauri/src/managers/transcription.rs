...
impl TranscriptionManager {
    ...
    fn load_model_unlocked(&self, model_id: &str) -> Result<()> {
        ...
        let loaded_engine = LoadedEngine::Parakeet(loaded_engine);
    ...
    /// Load a model without allowing a panic in the inference backend to leave
    /// the manager permanently stuck in its loading state.
    fn load_model_safely(&self, model_id: &str) -> Result<()> {
        match catch_unwind(AssertUnwindSafe(|| self.load_model(model_id))) {
            Ok(result) => result,
            Err(payload) => {
                ...
                let error_message = format!("Model loader panicked: {panic_message}");
                let model_name = self
                    .model_manager
                    .get_model_info(model_id)
    ...
    /// Ensure the selected model is ready. If another thread is loading it,
    /// wait for that attempt; otherwise load it on the current thread.
    pub fn ensure_model_loaded(&self) -> Result<()> {
        let model_id = get_settings(&self.app_handle).selected_model;
        if model_id.trim().is_empty() {
            return Err(anyhow::anyhow!(
                "No transcription model is selected. Please select a downloaded model."
            ));
        }
        ...
        if let Some(_loading_guard) = self.try_start_loading() {
            self.load_model_safely(&model_id)?;
        } else {
    ...
    /// Kicks off selected-model loading in a background thread. The loading
    /// guard is moved into the thread so it always resets the state and wakes
    /// waiters, including when the model backend panics.
    pub fn initiate_model_load(&self) {
        let model_id = get_settings(&self.app_handle).selected_model;
        if model_id.trim().is_empty() || self.is_model_loaded_for(&model_id) {
            return;
        }
        ...
        thread::spawn(move || {
            ...
            if let Err(e) = self_clone.load_model_safely(&model_id) {
    ...
    pub fn transcribe(&self, audio: Vec<f32>) -> Result<String> {
        ...
        // Never enter inference without the selected model. This also makes
        // direct callers (imports and retries) safe even if they forgot to
...
pub fn apply_accelerator_settings(app: &tauri::AppHandle) {
    ...
    // Only ORT accelerator is relevant for Parakeet (ONNX Runtime) models
...
/// Only ORT accelerators are available in this Parakeet-only build.
