...
fn initialize_core_logic(app_handle: &AppHandle) {
    ...
    // Warm the selected model at startup so the first dictation does not race
    // model initialization. "Immediately" remains genuinely on-demand.
    if get_settings(app_handle).model_unload_timeout != settings::ModelUnloadTimeout::Immediately {
        ...
        .setup(move |app| {
            ...
            let mut win_builder =
                tauri::WebviewWindowBuilder::new(app, "main", tauri::WebviewUrl::App("/".into()))
                    .title("Footy")
                    // Linux/GTK can occasionally create the first visible surface with a
                    // stale input region, making native titlebar buttons inert until a
                    // manual resize. Build one pixel larger, then immediately settle to
                    // the intended size below to force the initial configure event.
                    .inner_size(681.0, 571.0)
