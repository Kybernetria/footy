...
function App() {
  ...
  // Listen for model loading and transcription failures and show a toast.
  useEffect(() => {
    const unlistenModel = listen<ModelStateEvent>(
      "model-state-changed",
      (event) => {
    ...
    const unlistenTranscription = listen<string>(
      "transcription-error",
      (event) => {
        toast.error(t("settings.history.transcriptionFailed"), {
